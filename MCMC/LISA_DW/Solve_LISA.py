...
def Log_signal(x,burn_index):
    ...
    x = 10**x
    E, Vbias, log10_h2Ext, alphaExt, log10_h2Gal, alphaGal, b, c = x
    ...
    # Varying signal
    args_DW1      = [E, Vbias, b, c]
    hOm_SignalDW  = hOm_DomainWall(f_LISA, args_DW1) 
    hOm_back      = hOm_ExtGW(f_LISA,[log10_h2Ext, alphaExt]) + hOm_GalGW(f_LISA,[log10_h2Gal,alphaGal])  
    hOm_total_var = hOm_SignalDW + hOm_back
    ...
    if burn_index == 11:
        ...
        f_indices_DW   = np.argwhere(np.diff(np.sign(np.log10(hOm_total_var) - Om_LISA))).flatten() 
        hOm_total_var  = hOm_total_var[f_indices_DW[0]:f_indices_DW[1]]
        ...
        df = pd.DataFrame({'MCMC Likelihood':[LTotal], 'βH':[betaH], 'α':[alpha], 'Tp':[Tp], 'E':[E],
                           'Vbias':[Vbias],'log10_h2Ext':[log10_h2Ext], 'alphaExt':[alphaExt],
                           'log10_h2Gal':[log10_h2Gal], 'alphaGal':[alphaGal], 'b':[b],'c':[c],
                           'prom':[prominences1], 'SNR':[np.sqrt(rhosq_DW)]})
        
        df.to_csv('./{}/LISA_DW.csv'.format(Data_p), sep=',', index=False, mode='a', header=False)
